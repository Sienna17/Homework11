...
#Map function
df_list<-Map(function(tile,type){
            out_df<-Tile_to_rectangle_df(tile)
            out_df<-dplyr::mutate(out_df,crime_type=type)
            
            return(out_df)
            },tile_list,tile_type) #a list of data frame
...
#MAPPING NOTES
# outlist<-Map(function(x,y,z),list1,list2,vec1)
